    ...
    for i in range(len(level)):
        ...
        for j in range(len(level[0])):
            if level[i][j] in ZERO_LIST:
                ...
                bin_level[b].append("X")
        b += 1
    return bin_level

# remove all mario / flagpole from input level, insert flagpole into correct position in new level
def clean_level(level, mcts):
    ...
    level[mario_pos[0]][mario_pos[1]] = "M"
    level[flag_pos[0]][flag_pos[1]] = "F"
    level[flag_pos[0] + 1][flag_pos[1]] = "#"
